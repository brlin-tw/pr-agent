...
The configured language applies to AI-generated text and to static messages for which PR-Agent ships a translation catalog. Static messages fall back to U.S. English when a locale or an individual message has not yet been translated. The model you are using must also have good support for the specified language.

Stable command names, configuration keys, structured-output fields, and persistent-comment identifiers remain in English.

PR-Agent currently ships a Traditional Chinese (`zh-TW`) catalog. Contributors can add locales under
`pr_agent/locales/<locale>/LC_MESSAGES/`; catalogs use the `pr_agent` gettext domain and must include both the editable
`.po` source and its compiled `.mo` file.
